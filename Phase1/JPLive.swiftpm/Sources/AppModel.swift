...
final class AppModel: ObservableObject {
    ...
    @Published var status = "오디오 파일을 열어 시작하세요."
    ...
    private func run(_ newInput: any AudioInput, capture: UUID, language: SourceLanguage) async {
        ...
        do {
            ...
            timerTask = Task { [weak self] in
                while !Task.isCancelled {
                    ...
                    guard let self, self.captureID == capture else { return }
                    let activity = self.audioThrough.flatMap { self.speechTimeline.activity(through: $0) }
        ...
        enqueueTranslations(ready, language: language); scheduleMorphology(ready + captions.suffix(1).map(\.id))
        input = nil
    ...
    private func rememberAudio(_ output: PCMChunk) {
        ...
        if var decision = output.decision {
            ...
            speechTimeline.append([decision])
        }
    }
    ...
    func stop() async {
        guard running, let task = runTask else { return }
        status = "중지 중…"
        task.cancel()
        await input?.stop()
        await task.value
    ...
    func translationLoop(_ session: TranslationSession, language sessionLanguage: SourceLanguage, revision: UUID) async {
        ...
        while !Task.isCancelled, sessionLanguage == language, revision == translationRevision {
            guard let id = translationJobs.take(language: sessionLanguage) else {
                do { try await Task.sleep(for: .milliseconds(200)) } catch { return }; continue
            }
